...
extension Data {
    func wifStringPureSwift(enclave: SecureEnclave) -> String {
        ...
        return "PVT_\(enclave.rawValue)_\(String(data: base58, encoding: .utf8)!)"
    }
    
    func wifString(enclave: SecureEnclave) -> String {
        return "PVT_\(enclave.rawValue)_\(String(data: base58CheckEncodedData(version: 0x80), encoding: .utf8)!)"
    }
}
...
struct PrivateKey2 {
    ...
    init?(mnemonicString: String, index: UInt32) throws {
        ...
        hdnode_private_ckd(&node, index)                 // 0   - Slot #0
